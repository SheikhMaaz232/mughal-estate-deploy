...
@section('content')
        ...
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center py-2">
                <div class="flex-grow-1">
                    <h2 class="fs-base lh-base fw-medium text-muted mb-0">@lang('messages.list-of-detail-accounts')</h2>
                </div>
                <a href="{{ route('detail-accounts.create') }}" class="btn btn-sm btn-primary">@lang('messages.add-detail-accounts')</a>
            </div>
        </div>
    </div>
    ...
    <div class="content">
        ...
        <form method="GET" action="{{ route('detail-accounts.index') }}">
            <div class="row">
        ...
        <div class="block block-rounded">
            <div class="block-content block-content-full">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($detailAccountListings as $index => $detailAccount)
                            <tr>
                                <td class="text-center">{{ $index + 1 }}</td>
                                <td>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->mainHead->name_ur ?? '-' : $detailAccount->mainHead->name_en ?? '-' }}
                                </td>
                                <td>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->controlHead->name_ur ?? '-' : $detailAccount->controlHead->name_en ?? '-' }}
                                </td>
                                <td>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->subHead->name_ur ?? '-' : $detailAccount->subHead->name_en ?? '-' }}
                                </td>
                                <td>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->subSubHead->name_ur ?? '-' : $detailAccount->subSubHead->name_en ?? '-' }}
                                </td>
                                <td>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->subSubSubHead->name_ur ?? '-' : $detailAccount->subSubSubHead->name_en ?? '-' }}
                                </td>
                                <td>
                                    {{ App::getLocale() === 'ur' ? $detailAccount->party->name_ur ?? '-' : $detailAccount->party->name_en ?? '-' }}({{ $detailAccount->party->cnic_no ?? 'N/A' }})
                                </td>
                                <td>
